...
// @version      1.0
// @description  Polling infoOrder dari browser (fast-lane), kirim ke VPS Mars. Jalan di tab ditznesia.com.
...
(function () {
  ...
  const VPS_INGEST_URL = "https://api.clowatch.com/api/ingest"; // ganti ke domain VPS lo
  ...
  async function pollOnce() {
    ...
    try {
      ...
      try {
        ...
        log("respons bukan JSON (mungkin ke-redirect / cookie expired)");
        return;
      }
      ...
      // Kirim ke VPS pakai GM_xmlhttpRequest (bypass CORS)
  ...
  function sendToVps(orders, dur) {
    GM_xmlhttpRequest({
      ...
      onload: function (r) {
        if (r.status === 200) {
          lastOkAt = Date.now();
          log(`relay OK (${orders.length} orders, fetch ${dur}ms)`, r.responseText);
        } else {
          log("relay gagal HTTP", r.status, r.responseText);
        }
      },
      onerror: function () {
        log("relay network error");
      },
      ontimeout: function () {
        log("relay timeout");
      },
